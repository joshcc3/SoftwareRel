...
stmt :: NFData a => Stmt String a -> Stmt String (State St' Mp)
stmt s = s =>> stmtAction
    where
...
procedureDecl :: NFData a => ProcedureDecl' a String a -> State St' (ProcedureDecl' Mp IntermId Mp)
...
toIntermediateForm :: NFData a => Program String a -> State St' (Program IntermId Mp)
toIntermediateForm (P (Program a vs ps))
      = P <$> (Program <$> (_mp <$> get)
